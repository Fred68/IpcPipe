...
namespace Fred68.CfgReader
	...
	public partial class CfgReader : DynamicObject
		...
		Fred68.GenDictionary.GenDictionary _dict;			// Dizionario generico
		...
		void ReadConfig(string filename)
			...
			try
				{
				//string? line;
				string line = string.Empty;					// Tolto nullable, ok anche per framework vecchi
		...
		/// </summary>
		/// <param name="removeFromDictionary">Remove copied entries from dictionary</param>
		/// <param name="errorUnusedEntries">Error if unused entries remain</param>
		public void GetNames(bool removeFromDictionary, bool errorUnusedEntries)
			{
			if(!ok)
			{
				_msg.AppendLine($"Errori nella configurazione, GetNames() annullato");
				return;
			}
			...
			if(removeFromDictionary)
			{
				if((_dict.Count > 0) && errorUnusedEntries)
				{
					_msg.AppendLine($"Remaining {_dict.Count} unused entries");
					_msg.AppendLine(_dict.Dump());
					ok = false;
				}
				_dict.Clear();
			}
